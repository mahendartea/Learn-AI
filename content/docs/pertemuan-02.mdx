...
```mermaid
quadrantChart
    title Empat Sudut Pandang Definisi AI (Russell & Norvig, 2010)
    x-axis "Proses Penalaran (Thinking)" --> "Manifestasi Perilaku (Acting)"
    y-axis "Sesuai Standar Rasional (Rationally)" --> "Menyerupai Manusia (Humanly)"
    quadrant-1 "Acting Humanly (Turing Test)"
    quadrant-2 "Thinking Humanly (Cognitive Science)"
    quadrant-3 "Thinking Rationally (Laws of Thought)"
    quadrant-4 "Acting Rationally (Rational Agents)"
    "Sistem Cerdas Modern (Fokus Utama)": [0.8, 0.2]
    "Sistem Kognitif Otak": [0.2, 0.8]
    "Logika Inferensi Formal": [0.2, 0.2]
    "Uji Imitasi Manusia": [0.8, 0.8]
```
